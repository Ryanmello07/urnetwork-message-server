...
| A ↔ B | Both depend on `connect/message` for the record codec; the server imports it via `replace ../` and on `connect/protocol/message.proto`, which A owns and B's arms populate (§10.1). **B imports `connect/message` and never `connect/messagegroup`**, which is what keeps an MLS implementation out of that binary (§2.2, §2.3, Spec B §5.3). | §2.4 |
...
| A-ASSUME-5 | The message server is trusted to be the single Delivery Service, so `connect/messagegroup` implements no client-side commit-conflict resolution beyond re-derive-and-retry | MASTER §9.3. If multi-server lands in V2 this becomes a real distributed-consensus problem. |
...
| 2026-09-11 | A-23 | **REVISION A-20 IS INVERTED: MASTER §8.1 is now the document that changes and §5.3 is unedited.** No rule of this document moves and no byte of §5.3 is touched; this row exists because a reversal of a recorded position must name the position it reverses. **A-20 ruled** *"MASTER §8.1 stands as written and Spec A §5.3 is the document that changes"* (`SPEC-LEDGER.md` item **128**, the ruling paragraph) — and on **2026-09-11** the owner amended **MASTER §8 and §8.1** to carry this document's `EPH` carve-out in MASTER's own voice, which is the opposite allocation. **The four facts the inversion was measured on, each reproduced before it was written down.** (1) **§5.3 states the rule and excludes `EPH` in the same paragraph** — *"So `EPH` is excluded from this rule, `messagegroup.SealRecord` keeps refusing it, and the class it is keyed under is ledger item 152's to rule. Nothing here is a licence to seal an `EPH` head under `K_durable`."* *(**CORRECTED IN PLACE 2026-09-11, the same day this row landed. "In the same paragraph" is FALSE; "in the same section" is what is true.** A-16's rule is that a revision row's claim about what a document says is corrected in place by a dated annotation that quotes what it said, and this is that annotation. §5.3 states the rule in prose at `§5.3:1248-1257` — and again in the Go block at `§5.3:1225-1231` — and carries the exclusion quoted above at `§5.3:1290-1299`: **47 lines below the rule paragraph's own statement of it at `:1250`, 49 below that paragraph's first line, and 70 below the Go comment at `:1227`**, with **five** paragraphs between them (the owner's reason, the accepted cost, the position rule, why the class-blind counter makes the pin safe, and the lifted refusal). **The inversion this row records is unaffected and is strengthened rather than weakened:** a §5.3 reader who stops at the rule does not reach the carve-out either, so neither document put the exclusion where a builder meets the rule — which is the harm the MASTER amendment closes, stated one document wider than this row stated it. The measurement, as opposed to the claim, is untouched: §5.3 does carry the exclusion, and it is still the only document that did. **AND THE BASE OF THE THREE DISTANCES, STATED 2026-09-11, THIRD PASS OF THAT DATE, BECAUSE THE SUBTRACTION AND THE ANCHOR PRINTED BESIDE IT DO NOT SHARE ONE.** 47, 49 and 70 are measured to **`§5.3:1297`**, the first line of the exclusion **sentence** quoted at the head of this annotation — not to `:1290`, the first line of the **paragraph** `§5.3:1290-1299` this annotation cites for it. From `:1290` the same three distances are **40, 42 and 63**. Both are true of §5.3 as it stands; what was missing was which line the arithmetic used, and it is the one number this row foregrounds as the measurement. Corrected in place under A-16's rule rather than by a new revision row, because no rule of this document moves and a new row would shift every anchor beneath it.)* (2) **MASTER stated it unqualified**, at §8's record listing (*"`ct_head` AEAD, always retained"*) and at §8.1's ratchet paragraph (*"always under the durable class, since it is always retained"*), **with no `EPH` annotation at either**, and named ledger item **152** exactly **once** in the whole document — in a list of items that *"stay filed and unruled"*. (3) **The shipped code follows this document**, statically and end to end: `connect/messagegroup/seal.go:119` refuses every non-`DURABLE` class on the seal path and `:387` refuses it again on the open path, at `connect` `72ffdbd`. (4) **Nothing in the tree catches the divergence** — widening that refusal to admit `PERMANENT` reddens **2 of 208** top-level `messagegroup` tests (**211** counting subtests), and both are the blanket refusal gates themselves: `TestOnlyTheDurableClassIsSealedUntilM16IsRuled` (`seal_test.go:563`) and `TestOpenRecordRefusesTheClassesAndTheBlobRungSealRecordRefuses` (`m1w1repairs_test.go:628`). Measured by exporting `72ffdbd` to a scratch tree and running `go test ./messagegroup/ -v` unfiltered on both sides; `connect` itself was not modified. **So the harm the amendment closes was never the rule and never this document** — it is that a second implementer building from the top document would seal an `EPH` head under `K_durable`, and neither the corpus nor the test suite would stop them. **It also spends A-21's strongest argument**, which that row states in as many words: *"No Spec B change and no MASTER rule change, which is the ruling's own strongest argument."* A MASTER rule changes here, and the argument is spent rather than quietly dropped. **Ledger item 152 is NOT ruled by this** — what class an `EPH` head is keyed under stays open, and it must be ruled in one sitting with ledger open item **M1-27**, because `K_eph[n][b][t]`'s window `t` has no unit, no origin and no clock in any document and `message.EphBucketSeconds(0)` answers `-1`, the same `noLadderValue` sentinel as the off-ladder bucket 6 (`connect/message/record.go:131`, `:143`, `:168-173`), so no `EPH` head has a computable key today whatever class it is assigned. **No wire byte, no derivation label, no rule of this document and no Go file in `connect` changed.** |
...
| 2026-09-22 | A-28 | **A BACKFILL, AND THE ONLY ROW HERE THAT IS NOT THE COMMIT IT DESCRIBES: the seven commits this table was owed and did not take — `10f0a39`, `28c04b2`, `4eddba1`, `368ef8d`, `a401b9b`, `21f22c7`, `5210f09` — and the mechanical gate that takes the next one.** Nothing in this document is amended by this row. Measured over the whole history at the commit that writes it: 36 commits have touched this document and **eight** appended no row, the last **six of them consecutively**, so the table ended at A-27 of 2026-09-15 while the document moved six more times. **`10f0a39`** — E2's snapshot key becomes `K_snapshot[n] ‖ nonce_snapshot[n] = HKDF-Expand(storage_root[n], "snap/v1", 56)`, M-15's second instance (the 32-octet expand derived no nonce), and §5.11's byte-identity query gains the two filters it did not work without: as first published it stopped after `sed 's-//.*--'`, which leaves a comment's indentation behind, so it emitted 25 lines for MASTER and this document and 33 for Spec B and `diff` reported changes — the claim was right and the artefact published to prove it did not run. **`28c04b2`** — `aad_mls` v2: `Protect` becomes `ProtectBound`, a **builder** rather than a value because the AAD names the generation and the seal chooses it inside, **and the seal pins it** — if the generation consumed is not the one the builder was handed it MUST refuse and emit no ciphertext; the construction order moves the length refusal to the front and onto the **framed** length and computes `head_commit` before the frame; §5.2's published signatures do not move; and the preimage is deliberately **not restated** here, because item **207** is the finding that `message_id`'s derivation landed in three documents at once — MASTER §8.4.2 is the one normative statement and a builder transcribes it. **`4eddba1`** — the kind-carrier ruling: record bodies are opaque to this layer **without exception**, the reaction stops being the one exception, §2.2's `tombstone.go` and `reaction.go` are **struck**, and the content grammar moves to `sdk/urmessage/kind.go` — `0x05`/`0x06` as two codes rather than a `u8 op` inside the body, and a **raw** 32-octet `target_message_id` rather than an LP one; the grapheme-cluster and pinned-set halves are **not** enforced yet (M1-41). **`368ef8d`** — `GapReason` gains `"unsupported"` (item **221**), the value that makes every later content kind additive rather than a format break: a record that opened, whose signature verified, carrying a kind code this build does not know keeps its position and its `message_id`, is not a failure, and does not count toward `ErrRecordAbandoned`. **`a401b9b`** — §8.2's `GroupHandle` transcription re-measured against `engine_test.go` and amended **to the block as measured**, 5 engine and 33 handle methods: it had stood at twenty-six since 2026-09-17 and was missing `PairwiseExport` (item 228), `CommitAdd` (239), the `ProtectBound` / `PeekSender` / five-result `Unprotect` trio its own comment described, and item 242's `PendingEpoch`, `PendingExport`, `DiscardProcessed` and the by-value commit arms. **`21f22c7`** — ruling 32: §12.1's **S7** gains `read_epoch` inside the `FetchAttestation` signature, and the tuple attestations are compared within becomes `(class_mask, heads_only, read_epoch)`. **`5210f09`** — ruling 27: §5.11 gains attachment kind **`0x0005`** `EpochDigest`, the six public `EpochAttachment` fields plus `LP(H(epoch_keys))`, with its `epoch_keys` preimage; §12.1's **S1** and its key-custody paragraph now name **both roads** — under kind `0x0001` `write_key[n]` rides inside the commit record's `server_attachment`, under `0x0005` the attachment carries the digest in its place and the key rides on the **request** beside the record (`SubmitRequest.epoch_keys`, `CreateGroupRequest.epoch_keys`), bound by that digest, so nothing the server serves back carries a key. **Why a backfill and not a reminder:** six consecutive skips is this project's own definition of a gate that has stopped tracking its property, so `editlog_test.go` now holds every commit that changes this document to adding a row **inside §0.6**, measured off the file as of that commit. The seven hashes above are dispositioned `backfilledInTheLog` there, which is asserted **against this row** — the test fails unless each hash appears in this region; `3bc0603`, which created the document, is dispositioned `createdTheDocument`, asserted by this table beginning at **A-2**. The check also runs against the working tree, so it goes red while an edit is still uncommitted rather than after the push. |
...
connect/                                   (existing; never imports its own children)
  ...
  message/                                 the SERVER-SAFE half of the storage layer: the record
                                           the message server parses, and the surface §12.1
                                           publishes to it. Imports connect and connect/mls/syntax,
                                           and NEVER connect/mls (see the note below this tree)
    record.go                              Record, RecordHeader, retention classes
    codec.go                               record wire encode/decode (uses mls/syntax)
    aad.go                                 the two record AAD preimages (MASTER §8)
    writeauth.go                           write_auth and req_auth MACs (§5.7)
    recovery.go                            Ed25519 recovery proof and its verifier (§5.7) — here
                                           and not in messagegroup because §12.1 publishes both
    attachment.go                          server_attachment encoding (§5.11)
    rendezvous.go                          §12.1's nine published rendezvous functions: the
                                           preimages, the five verifiers, RendezvousId,
                                           DepositVerifyKey, RendezvousDepositBytes (§5.14)
    pad.go                                 size buckets. COVER records and the body padder are
                                           messagegroup's: the ladder is authenticated and the
                                           padding is done under a record key
    errors.go
  messagegroup/                            the CLIENT half. imports connect, connect/message and
                                           connect/mls (peers). The message server never imports
                                           it, and Spec B §2.2's allow list does not carry it
    keyschedule.go                         storage_root, class keys, record_key ratchet
    ratchet.go                             sender ratchet + skipped-key window
    streamindex.go                         the StreamIndexReserver interface (§5.6); the durable
                                           implementation is sdk's, per §8.2
    xwing.go                               X-Wing KEM (§5.4) — the file that carries the
                                           connect/mls edge, and the reason for the split
    wrap.go                                epoch wraps: device leaves, recovery targets, snapshot
    handle.go                              sender_handle, recovery_handle, wrap_target_handle
    session.go  seal.go                    GroupSession, SealRecord, OpenRecord (§5.2)
    engine.go                              the GroupEngine interface (§6), EngineProcessed, and the
                                           connect/mls adapter — all three in one package, because
                                           stagedRef is unexported and only the declaring package
                                           can populate it
...
**Why the storage layer is two packages and not one.** The message server links `connect/message`
(§12.1, and Spec B §2.2 allows it by name). Spec B §5.3 is normative that *"the message server
binary MUST NOT link an MLS implementation"*, and Spec B §13 item 8 asserts it over the package
`github.com/urnetwork/connect/mls`. One package cannot satisfy both: `xwing.go` needs `connect/mls`'s
four reviewed X25519 wrappers (§5.4, guardrail G3), so the first server package that parses a
record drags `connect/mls` into the closure behind it. That is not hypothetical — the message
server's own dependency gate failed on exactly it, with `connect/message` measured as the single
direct importer in a 481-package closure and `xwing.go` as the single importing file. The split is
therefore by **capability**: `connect/message` is everything the server may reach and reaches no MLS
type; `connect/messagegroup` is everything else, and its `connect/mls` import is its ordinary case
rather than a tolerated exception.

**The name is load-bearing.** `messagegroup` is a **sibling** of `message` and not a child. Spec B
§2.2's allow list covers `connect/message` and everything beneath it, so a client half at
`connect/message/group` would be linkable by the server with the dependency gate silent — measured
in both directions against a working copy. A sibling is outside that allowance, so the day a server
package imports it the gate fails by name and a human looks, which is the behaviour the rule exists
for. Do not relocate `messagegroup` under `message`.

### 2.3 Layering rules (enforced in CI)

```
sdk  ──imports──▶  connect/messagegroup  ──imports──▶  connect/mls  ──imports──▶  connect/mls/syntax
                      │                                                           ▲
                      └──imports──▶  connect/message  ────────────────────────────┘

sdk, connect/messagegroup and connect/message may each import connect.
connect imports none of the three.
```

Forbidden edges, each asserted by a test in `connect/layering_test.go` and `sdk/layering_test.go`
that walks `go list -deps`:

- `connect` → `connect/mls`, `connect/message` or `connect/messagegroup` (A2)
- **`connect/message` → `connect/mls`** — the edge the §2.2 split exists to forbid, and the one
  a refactor breaks by accident. `connect/message` is the package the message server links; Spec B
  §5.3 is normative that that binary links no MLS implementation and Spec B §13 item 8 asserts it
  over the package. `connect/mls/syntax` is allowed and is not an MLS implementation — no MLS type,
  no key schedule, no validation semantic
- **`connect/message` → `connect/messagegroup`** — the split is one way. The client half knows
  about the record; the record does not know about the client
- `connect/mls` → `connect`, `connect/message` or `connect/messagegroup` (MLS must be a
  self-contained crypto library, so it can be audited and fuzzed without the transport)
- `connect/mls/syntax` → anything but stdlib
- `connect/message` or `connect/messagegroup` → `sdk`
...
are accepted; **and the sentence that used to discharge the second cost does not, which is recorded
here rather than left to be rediscovered.** It read *"the ephemeral guarantee is unaffected, because
`eph_root` is time-sliced and its window closes on the timer regardless of which epochs are retained
(§5.3)"* — **amended 2026-09-13, second pass of that date.** The 2026-09-13 window ruling puts `t` at
the **derived** key: `K_eph[n][b][t] = HKDF-Expand(eph_root[n], "eph/v1" ‖ u8(b) ‖ u64(t), 32)`, where
`eph_root[n]` is one 32-octet value per epoch and `t` is an HKDF `info` term, so the root has no window
of its own and `DeleteGroupStateBefore` **is** the only deletion the corpus names for it. A retained
epoch is therefore a retained `eph_root`, and thirty-two of them is thirty-two — against a device that
retains the state, which is what `PastEpochWindow` is about. It is unchanged against the three
adversaries MASTER §8.1 now names (a server that never holds `eph_root`, a device provisioned tomorrow,
a seedphrase holder), because none of them holds the epoch state either. **What is owed is a
destruction schedule for `K_eph[n][b][t]` and `eph_root[n]` in the window unit the ruling defined:
ledger open item 186**, filed and not ruled. The 32 is **not** revisited here, because the number is a
product decision about history and the item is about key lifetime.
...
## 5. `connect/message` and `connect/messagegroup` — the storage layer
...
**The storage layer is two packages, and every file annotation below names which.** `connect/message`
is the server-safe half — the record the message server parses, the four preimages it
authenticates one with, and the surface §12.1 publishes. `connect/messagegroup` is the client half
— the key schedule, both ratchets, the session, the sealer, the wraps and the MLS adapter. The
boundary is a capability rather than a convenience: Spec B §5.3 forbids the message server binary an
MLS implementation, `connect/mls`'s four reviewed X25519 wrappers are what X-Wing (§5.4) is built
on, and one package cannot both be linked by that binary and reach them. §2.2 carries the tree and
§2.3 the forbidden edges.
...
type RecordHeader struct {
    ...
    EphWindow        uint64    // t, the eph ladder's time-slice. PLAINTEXT and on the wire
                               // (MASTER S8). ALWAYS ENCODED; zero on PERMANENT, DURABLE,
                               // MEDIA and EPH(0). Sender-computed from its own sent_at;
                               // an opener takes this value and never recomputes it.
                               // Added 2026-09-13 (A-25, ledger 152 / M1-27) -- a NEW
                               // FIELD in a wire format section 14 froze, which is why
                               // format_version becomes 0x02.
    SizeBucket       SizeBucket
    ExpireAt         uint64    // unix MILLISECONDS, 0 = unset. May only shorten retention.
    BodyHash         [32]byte  // H(CtBody). RETAINED after CtBody is erased for
                               // PERMANENT, DURABLE and MEDIA; ZEROED for EPH(1..5)
                               // at prune_after (Spec B §7.2, MASTER §8). Amended
                               // 2026-09-11 (A-24); it read "RETAINED after CtBody
                               // is erased" with no class in it.
...
type Record struct {
    ...
    Header    RecordHeader
    CtHead    []byte        // AEAD. Retained for PERMANENT, DURABLE and MEDIA; nil
                            // once an EPH(1..5) record is pruned, because Spec B §7.2
                            // sets ct_head = NULL at prune_after. Amended 2026-09-11
                            // (A-24): this comment was MASTER §8's PRE-AMENDMENT
                            // wording, "AEAD, always retained", verbatim -- the rule
                            // §5.3 has excluded EPH from since A-20, restated here
...
**On the bucket-0 sentinel, and this document is the one it lands as work in.** `EphBucketSeconds` is
`connect/message`'s, which is this spec's half of the system, so the sentinel rule is the only line of
the 2026-09-13 second-half ruling that becomes code here: `record.go`'s table answers `-1` for bucket 0
and `-1` for `6..255`, two meanings under one sentinel, and **m1 open item M1-27** carries the change as
a later dispatch against `connect`. The negative for `6..255` is **unreachable through a parsed record**
— `RetentionClassOf` refuses every wire byte outside `0x10..0x15` — so it is a programmer-error sentinel
rather than a value a record can carry, while `0` for bucket 0 is a real retention window a caller may
act on. *(**The paragraph inside the block was added here 2026-09-13, second pass of that date.** On the
first pass it went into MASTER §8 and Spec B §3.1 and **not into this document**, and the two copies that
did exist did not match each other — which is precisely the divergence the sentence above this block
declares the block exists to stop, and ledger item **141**'s class. All three copies now carry one text;
each document's own commentary is outside the fence so that it cannot diverge again.)*
...
```go
// messagegroup/seal.go — NOT codec.go: `connect/message/codec.go`'s header states it exports
// nothing beyond the three functions §12.1 publishes, and SealRecord/OpenRecord are on no line
// of §12.1 because the server never seals and never opens.
type recordBuilder struct{ ... }   // unexported; the only builder inside this package
...
`Record` has no exported constructor, and the builder above is unexported so that nothing in
`connect/messagegroup` assembles one out of order. **That is a discipline inside the package and not
a guarantee of the language:** `Record`'s fields are all exported, so a keyed composite literal from
any package builds one, and no gate can stop it. The property the builder buys is that the *ordered*
path is the only one anybody writes, which is what §5.2 is for; a reader who takes it for a
compiler-enforced invariant will be wrong in the same way §6's `stagedRef` sentence was.
...
```go
// messagegroup/keyschedule.go — the whole of this block is in the client half. §12.1 gives the
// message server "no key-schedule function", and after the §2.2 split that is a fact about which
// package it links rather than a rule somebody has to remember.
...
// K_eph[n][b][t] = HKDF-Expand(eph_root[n], "eph/v1" || u8(b) || u64(t), 32)   MASTER S8.1
//
// window IS t, AND IT IS THE RECORD'S OWN eph_window FIELD -- ruled 2026-09-13, M1-27.
// The SENDER computes it, once, as
//     window = floor(sent_at_ms / (EphBucketSeconds(bucket) * 1000))     for bucket 1..5
//     window = 0                                                        for bucket 0
// origin the Unix epoch, unit a count of whole buckets, clock the sender's own -- the
// same wall-clock reading it puts in sent_at. It is then written to record_bytes in the
// clear (MASTER S8), covered by write_auth and by both AADs. AN OPENER TAKES THE WIRE
// VALUE AND NEVER RECOMPUTES IT. This function does not read a clock and must not: a
// window it derived itself would be a different window from the sender's on every
// record that crossed a boundary, and the AEAD would be the only thing that said so.
...
//
// THE TWO record_key[i] ARE THE SAME LADDER'S, AT THE SAME POSITION -- the ladder rooted
// at the RECORD'S OWN class key, for every class. RULED 2026-09-13, which REVERSES the
// 2026-09-07 ruling this comment carried until then: "RecordAeadHead is handed a
// record_key from the ladder rooted at ClassKeys.Durable -- ALWAYS, whatever the record's
// own retention class". See the prose below for what was ruled, why it was wrong and what
// replaces it. The two AEADs are separated by their HKDF LABELS, "rec/v1/head" against
// "rec/v1/body", which is what MASTER I7's "distinct keys and distinct AADs" has always
// meant, and it is why one position is safe.
//
// The signatures still do not change and still cannot express it: both take a 32-octet
// secret, so the binding is at the CALL SITE and the call site is SealRecord/OpenRecord.
// What a caller can now get wrong in silence is the INVERSE of what it was -- passing the
// durable ladder's record_key to RecordAeadHead for a non-DURABLE record is a working
// program that seals an EPH header under a key nothing destroys.
func RecordAeadHead(recordKey []byte) (key, nonce []byte)
func RecordAeadBody(recordKey []byte) (key, nonce []byte)

// messagegroup/handle.go — §2.2's tree puts these two in handle.go, not here.
...
//
// CALLED ONCE, AT GROUP CREATION. What a member persists for the life of the group is
...
**`ct_head` IS SEALED UNDER THE RECORD'S OWN CLASS RATCHET, WHATEVER THAT CLASS IS — exactly as
`ct_body` is. RULED 2026-09-13, and it REVERSES the ruling of 2026-09-07 this paragraph carried until
then.** **The rule, normatively:** `SealRecord` and `OpenRecord` derive **both** `key_head ‖
nonce_head` and `key_body ‖ nonce_body` from **one** `record_key[i]`, on the ladder rooted at
`HKDF-Expand(class_key = the record's own class key, "sender/v1" ‖ LP(leaf_index), 32)`, at **one**
position. For an `EPH(b)` record that class key is `EphKey(ephRoot, b, window)` and the window is the
record's own `eph_window` field. **THE RULE HAS ONE EXCEPTION AND IT IS NAMED HERE RATHER THAN ONLY
WHERE IT LIVES** (added 2026-09-13, second pass of that date): **a device-wrap record takes no class
key at all.** Its `record_key[0]` is rooted at `env_key[k]`, the MLS-exporter envelope — §5.11 (1) and
MASTER §8.2 — and that is not a qualification of this rule so much as a different root for one record
class. It is stated at the rule rather than only in §5.11, because the rule is what an implementer
transcribes, and because the
`eph_root` device wrap is an **`EPH(5)`** record whose payload **is `eph_root[k]`**: a builder applying
the sentence above to it literally would call `EphKey(ephRoot, 5, window)` to open the record that
*delivers* `ephRoot`, which is the `sent_at` circularity that killed the cheap answer to `M1-27`,
reappearing one level in. The two AEADs are separated by their HKDF labels, `"rec/v1/head"`
against `"rec/v1/body"`, which is what MASTER **I7**'s *"distinct keys and distinct AADs"* has always
meant and is why one position is safe. Ledger item **152** closes with this, and ledger item **128**
closes with it.

**What was ruled on 2026-09-07, why it was wrong, and what replaces it — all three, because this is a
reversal and a reader who finds either half alone is owed the other.** *Ruled:* revision **A-20**, m1
open item **M1-6**, ledger item **128** — *"`ct_head` is sealed under the DURABLE class ratchet,
whatever the record's own retention class"*, with `RecordAeadHead` taking the ladder rooted at
`ClassKeys.Durable` always and `RecordAeadBody` the record's own, coinciding only for a `DURABLE`
record. *Its reason:* *"the head is always retained, so it is keyed by the class that is always
...
head is not always retained, and the failure the rule existed to prevent — a retained header going
unopenable when its body vanishes — is a failure only where the head outlives the body, which for
`EPH(1..5)` it does not. And it was ruled on item 128's own narrower terms, a two-ratchet bookkeeping
contradiction, **without ledger item 152 beside it although 152 had asked in those very terms that it
be**: 128 filed the bookkeeping and never named the confidentiality consequence. *What replaces it:*
the paragraph above.

**`PERMANENT`, `DURABLE` and `MEDIA` are unchanged in effect; the whole of what moves is `EPH`.**
`ClassKeys.Perm`, `ClassKeys.Durable` and `ClassKeys.Media` all descend from `storage_root[n]` and
none is ever destroyed, so for those three the repair costs nothing and changes no guarantee. An
`EPH(1..5)` record's metadata — the MLS `PrivateMessage` header, `type` and `sent_at` — now dies with
`K_eph[n][b][t]` instead of living under a key every member, every future device and every seedphrase
holder holds forever.

**And the consequence that is the point.** Before this ruling the only thing stopping an `EPH` head
outliving its timer was **a cooperating server** — Spec B §7.2's `ct_head = NULL` sweep, an
*operational* erasure, against the adversary MASTER §8.1 names as *"retained server ciphertext"*: a
backup, a replica that missed the sweep, a legal hold, a seized snapshot. **The ruling converts that
guarantee from behavioural to cryptographic**, the same conversion ruling 3 of 2026-09-13 made for
`eph_root[n]` and left the head out of. *(Qualified 2026-09-13, second pass of that date: **against
retained server ciphertext, a newly provisioned device and a seedphrase holder — and NOT against a
seized member device.** `K_eph[n][b][t]` hangs off one per-epoch `eph_root[n]` with `t` as an HKDF
`info` term, so a device holding that epoch's state recomputes every window's key and now reads `t`
off the record in the clear; no document schedules `eph_root[n]`'s destruction. **Ledger open item
186**, filed and not ruled. It does not touch the reversal, which is a strict improvement against
every adversary — it bounds what the reversal BOUGHT.)*

**THE COST THE REPLACED RULING ACCEPTED IS NOT PAID BECAUSE IT IS NO LONGER INCURRED.** It read: *"A
non-`DURABLE` record draws its head and its body from two different ratchets, so one record's single
`stream_index` covers two ratchet positions, and this section said which position each takes
nowhere."* Under one ladder at one position there is one `stream_index` covering one position and the
contradiction dissolves. **The position rule below is NOT withdrawn** — `i = stream_index` in every
ladder, class-blind, ruled as shape **A1** — because it also answers the device wrap's own instance,
where two records of two classes sit on one `env_key[k]` root (§5.11 (1)); that instance is
untouched by this reversal and still needs A1.

**THE POSITION RULE, RULED 2026-09-07 as shape A1 — ledger items 143 and 169, ruled together.**
`i = stream_index` in **every** ladder, and the `stream_index` counter is **class-blind**, one per
`(group_id, sender_handle)` (§5.6). Uniqueness of `i` therefore follows from §5.12 step 6's existing
*"MUST NOT be reused"* rather than from a second, unwritten discipline. *(**This sentence read "the
head takes position `stream_index` on the durable ladder and the body takes position `stream_index`
on its own class ladder" until 2026-09-13.** Under that day's reversal head and body take **one**
ladder — the record's own — at one position, so the two-ladder clause names a shape that no longer
exists. **A1 itself is untouched and is still needed**: its own load-bearing case is the device wrap,
where two records of two classes sit on one `env_key[k]` root that carries no class at all, and the
reversal does not reach that.)*
...
the message server the Poly1305 one-time key. *(**That particular collision is gone with the 2026-09-13
reversal** — the two records now take position 5 on two different class ladders, `K_durable` and
`K_perm`, so they never share a key. The argument is kept because the reversal does not reach the case
A1 exists for, the device wrap's shared classless root, and because a reader meeting A1 elsewhere needs
to know which of its two instances survived.)* **A1 removes the byte**, so one sender's classes draw
...
**THE REFUSAL IS NOW LIFTED IN FULL.** It read *"A builder MUST NOT seal a non-`DURABLE` record
before 169 is ruled"*, was lifted on 2026-09-07 for `PERMANENT` and `MEDIA` only, and then stood for
`EPH` alone under ledger item **152**. **152 is ruled** (2026-09-13, the reversal above), so
`SealRecord` and `OpenRecord` may seal and open **every** retention class. **`EPH` is no longer a
special case in this section at all** — it takes its own class key like the other three, and the
sentence *"Nothing here is a licence to seal an `EPH` head under `K_durable`"* that stood here is
withdrawn because nothing keys any head under `K_durable` any more except a `DURABLE` record.

**Two preconditions travel with the lift, and a builder that ignores either has a working program
that is wrong.** *(1)* An `EPH` record's class key is `EphKey(ephRoot, bucket, window)`, and `window`
is the record's **own `eph_window` field**, not a value the sealer or the opener reads a clock for —
see the declaration above and MASTER §8.1. **Except for a device-wrap record, which is rooted at
`env_key[k]` and calls no `EphKey` at all** (added 2026-09-13, second pass of that date; §5.11 (1)):
that carve-out matters here and not only in §5.11, because the `eph_root` device wrap is `EPH(5)` and
this sentence unqualified tells its builder to key it with `EphKey(ephRoot, 5, window)` — the record
whose payload IS `eph_root[k]`. **And what `eph_window` VALUE that record carries is not ruled; a
builder MUST NOT publish it until it is** — ledger open item **185**, MASTER §8.2, and Spec A **S19**
carries no carve-out for it either. *(2)* **An opener MUST refuse an `EPH(1..5)` record whose
`eph_window` is more than one window AHEAD of its own clock**, with a typed error separable by
`errors.Is` from every AEAD failure, and render it as a `Kind == "gap"` with `GapReason ==
"malformed"` (§7.4). A window behind the opener's own is **not** a refusal in any amount: the opener
derives the key for the wire window and either still holds it or has destroyed it on schedule, and a
destroyed one is a gap with reason `"expired"`. **The asymmetry is the whole defence.** The opener can
derive *any* window's key from `eph_root[n]` — an HKDF-Expand takes whatever `t` it is given — so a
sender that claims a far-future window would make the record openable long past its timer on a client
that honoured it, and the server-side check (Spec B §5.1, §7) is the only other thing standing there.
A client that trusted a far-future window would quietly disable disappearing messages for every record
a hostile sender or a hostile server put in front of it, and §12.4's required string would be false
with nothing anywhere reporting it.

**What a member persists for the life of the group is `group_handle_key`, not `storage_root[0]`.**
`GroupHandleKey` is evaluated **once**, when the group is created, and its 32-octet answer is what a
device durably keeps and reloads at every later epoch; `storage_root[0]` is dropped with the rest of
epoch zero's schedule. The distinction is worth a normative sentence because both values are 32
octets and the wrong one is a **working program**: MASTER §8's obligation is about what a member
*holds* — *"a member that does not hold it cannot compute its own handle and therefore cannot
write"* — and `storage_root[0]` is strictly more than that, since every class key and epoch zero's
`write_key` and `read_key` expand from it. Keeping a whole epoch's key schedule forever in order to
recover a **public** routing identifier that every member of the group can already compute is a
retention decision nothing in this design asks for. `messagegroup.NewGroupSession` takes the kept
value as `groupHandleKeyEpoch0` and refuses any width but 32 with a typed error; the parameter is nil
only for a session opened at epoch 0, where the current root **is** epoch zero's and the constructor
expands it itself.

**The inverse mistake is not defended, and this paragraph is the only defence there is.** A caller
that hands the constructor `storage_root[0]` where it wants `group_handle_key` is accepted in
silence — the width matches, every key of the session derives cleanly, and every round trip that
device makes with itself succeeds — while its records carry a `sender_handle` no other member
computes and no peer's receiver ratchet matches. **Ledger open item 167** carries it, filed and not
ruled, with the three candidate refusals and what each costs.
...
input, and none in `connect/message` at all. MASTER
...
```go
// messagegroup/xwing.go — the client half. This file is the one that imports connect/mls,
// for the four reviewed X25519 wrappers below, and its import is why the storage layer is two
// packages: a message server that linked it would link an MLS implementation, which Spec B
// §5.3 forbids. The server never wraps, never unwraps and never holds an X-Wing key.
const (
...
MUST NOT be referenced anywhere in `connect/mls`, `connect/message`, `connect/messagegroup`, or
`sdk/message_*.go`.** The grep gate's scan roots must name `connect/messagegroup` from the day it
exists; a confinement gate whose root is missing reports clean having read nothing. A CI
...
one. *(**The clause that stood here is VOID as of 2026-09-13** and is corrected rather than annotated,
because it names a consequence that can no longer occur. It read: *"and note that giving transients
their own counter re-opens ledger item 169's collision for `EPH` heads on the day ledger item **152**
rules the `EPH` classes onto the durable root."* **Ledger 152 ruled the opposite:** an `EPH` head takes
`K_eph[n][b][t]`, its own class key, and is never on the durable root, so no ruling can now put it
there. Two counters over one root is still the shape A1 removed, and it is still the reason a second
counter is not granted here — but `EPH` heads are not the instance. **M1-25's real cost, the
1,025-transient `out_of_window` wall above, is untouched.**)*
...
```go
// messagegroup/streamindex.go — the reserver is not a ratchet, and it is ordered before one.
...
```go
// message/writeauth.go — the server-safe half: §12.1 publishes every name in this block.
...
//     ‖ u8(retention_class) ‖ u64(eph_window) ‖ u8(size_bucket) ‖ u64(expire_at)
//     ‖ LP(H(ct_head)) ‖ LP(body_hash) ‖ LP(blob_id) ‖ LP(H(server_attachment)))
//
// u64(eph_window) added 2026-09-13 (A-25), immediately after u8(retention_class), the
// field it qualifies. It is in the preimage because the SERVER ACTS ON IT -- MASTER I6 --
// and the act is the +/- one window refusal of spec B section 5.1 and section 7.
...
unchanged and still carries `pq_secret[n]` and `eph_root[n]`"* was true of a single device-wrap record.
The owner's third M1-1 ruling splits that record in two — a `PERMANENT` record carrying `pq_secret[n]`
and an `EPH(5)` record carrying `eph_root[n]` — for the reason §5.11 gives. E1's first sentence, the
correction itself, is untouched: the **recovery** wrap still carries `storage_root[n]` and
`archive_secret[n]` and still carries no `pq_secret`. E2 and E3 are untouched entirely.

Sizing after all three corrections and after the 2026-09-13 rulings, for 500 members × 2 devices: every
wrap still pads to `size_bucket 2` (a `ct_body` of exactly 4,112 bytes), so each wrap record is about
4.6 KB on the wire — device-wrap records 9.2 MB across two records per leaf, recovery wraps 2.3 MB,
snapshot 0.30 MB on the bulk plane, **≈ 11.5 MB per commit** and ~90 round trips at
`max_submit_bytes = 131072`. §5.11's sizing paragraph carries the arithmetic. The pre-ruling figures
were 4.6 MB, ≈ 6.9 MB and ~55 round trips.
...
field for field"*, which is measurably false and overstates the thing it is warning about — the danger
of a scope claim in a *transcribe from here* sentence is that a reader who checks one field and finds it
identical stops believing the sentence. The measurement, with its query, so it can be re-run:
...
`durable_ttl_seconds`' annotation is longer here because it carries `RetentionApplied.durable_clamped_down`
and Spec B §7.3 case 3's no-refusal rule, which MASTER states elsewhere.) **One field of eight, and it
is the one whose value opens an epoch** — which is a sharper warning than *field for field*, not a
milder one.
...
EpochAttachment {
    ...
    u32  expected_wrap_count    // RULED 2026-09-13: the device-wrap RECORDS plus the one snapshot,
                                ...
                                //   expected_wrap_count counts, and what it cannot see" below, which
                                //   also says what detects a missing recovery wrap (nothing does).
}
...
fan-out executed against the shipped server at all**: `exemptFromEpochComplete`
(`msgrepo/store/memory.go:937`) exempts exactly `AttachmentWrap` and `AttachmentEpochComplete`, so an
`AttachmentRecovery` record submitted while the fan-out is open is refused `REASON_EPOCH_INCOMPLETE`
(`memory.go:582`). That is not a gap awaiting a one-line addition: `store/contract.go:2555`,
`OnlyTheExemptKindsPassTheGateWhileTheFanOutIsOpen`, derives the class it partitions from the declared
`AttachmentKind` constants **in both directions** and runs it against **both** store implementations,
asserting the recovery refusal by name. The other available fix — exempting `AttachmentRecovery` — was
rejected: it reverses a green, derived, two-store assertion and widens the set of records that may land
while a group is not writable.

**Nothing in the store gate changes, and that contract test stays green as written.** Verified rather
than assumed: the test asserts what the server does with a recovery record submitted *while a fan-out
is open*, and this ruling does not change that answer. What changes is that a conforming client no
...
**May a stranded recovery wrap be republished at a later epoch? The cryptography permits it, the wire
costs nothing — AND THE PROCEDURE THIS SECTION PUBLISHED FOR IT WAS UNSAFE. A conforming implementer
following the 2026-09-14 text would have shipped an XChaCha20-Poly1305 nonce reuse.** Answered here
rather than left as an exercise, because a retry is the first repair an implementer reaches for and it
is the only mitigation the window above has.

**CORRECTED 2026-09-15. Three consecutive passes priced this retry and each supplied, in its own text,
the fact that defeated its own conclusion.** The first ruled a retry *unaddressable* because
`RecoveryTag` carries no epoch, at a price of an **A6 wire-format change** across three documents. The
second showed the restorer can separate candidates by trial decryption and re-priced it at **zero wire
bytes and two normative sentences** — that half stands and is re-derived below. **What the second pass
did not price is the seal**, and the seal is where the retry actually costs something.

**THE FACT ALL THREE PASSES HAD IN FRONT OF THEM. `AAD_head` binds the RECORD's epoch; the wrap head's
key and nonce bind the CONTENT epoch; a republish moves the first and cannot move the second.** Three
derivations, none of them new, read together for the first time here:
...
So the recovery wrap's `(key_head, nonce_head)` is a pure function of `ss` and the content epoch, and
it moves with **neither** of the two `AAD_head` fields the shipped server forces a republish to change:
`store/memory.go:578` refuses any record whose epoch is not the current one — which is why the wrap was
stranded in the first place — and `store/memory.go:610` refuses a `stream_index` that is not strictly
greater, which §5.7's outbox rule already requires (*"re-sealed at the new epoch, consuming a"* **fresh**
*"`stream_index`"*). **A republisher that rebuilds the record around a `ct_xwing` it kept therefore
seals a second, different `AAD_head` under a byte-identical `(key, nonce)`.** That is keystream reuse
plus recovery of the Poly1305 one-time key — header forgery over a preimage covering `body_hash`,
`blob_id` and `H(server_attachment)` — and it is the exact failure part (5) below names, in its own
words, as *"a repeated pair under XChaCha20-Poly1305 leaks the Poly1305 one-time key"*.

**And §5.9's mechanical defence does not reach it.** Guardrail **G5**, *"AEAD nonce reuse"*, is defended
by *"§5.6 durable reservation + `TestStreamIndexNeverReused`"*. That guards `i` in `record_key[i]`,
which is what an ordinary record's head nonce descends from. The recovery wrap's head is keyed from
...
- **The restorer can still tell two candidate wraps apart, and that half of the 2026-09-14 correction
  ...
  *inside* `wrap_key`'s HKDF `info` (`… ‖ u64(epoch) ‖ u8(target_type) ‖ LP(target_id) ‖ …`,
  ...
  first candidate whose `ct_head` opens is the content epoch. **The property that makes this cheap,
  stated so it stays checkable: the decapsulation count does not depend on the bound at all**, because
  `ss` is epoch-independent — the search adds symmetric trials (two HKDF-Expands and one AEAD open each)
  and **no** asymmetric operation.
- **A wrong candidate opens with probability about `2^-128`, and the 2026-09-14 text stated that
  inverted.** It read *"every other fails"* / *"Poly1305 with probability `2^-128`"* — two strings,
  because it straddled a line break — and it says a wrong guess almost always **succeeds**. A wrong
  candidate **fails** with probability `1 − 2^-128`; `2^-128` is the chance it opens anyway, and that
  direction is the one the search's correctness rests on. The same inversion was written into ledger
  item 142 and is corrected there.
- **THE REUSE IS A PROCEDURE CHOICE AND NOT A FORMAT CONSTRAINT, and this is the fact the seal analysis
  turns on.** `XwingEncapsulate` **cannot be derandomized**: `crypto/mlkem`'s `Encapsulate` takes no
  randomness argument and reads `crypto/rand` itself, so the ML-KEM half is fresh whatever reader the
  caller supplies (`connect/messagegroup/xwing.go:236`, and asserted rather than documented by
  `xwing_test.go:277`, `TestXwingEncapsulateIsNotDerandomizable`). A republisher that **re-encapsulates
  from the wrap plaintext** therefore gets a fresh `ss`, a fresh `wrap_key` and a fresh
  `(key_head, nonce_head)` — unconditionally, and without depending on any field of the AAD, on any
  server check, or on anything the restorer does. A republisher that **reuses a stored `ct_xwing`** gets
  the collision above. Both conform to every wire rule in this document; the difference is entirely in
  what the outbox kept.
- **So a bare MUST is the wrong shape for it, and §5.9 already says why.** No party can check
  compliance: the server never decrypts, the restorer sees only the record that landed, and the
  ciphertext the reuse would be compared against was **refused** and exists only on the server's side of
  the wire. §5.9's own idiom is the answer — G4 does not tell an implementer to keep `body_hash` out of
  `AAD_body`, it makes `AAD_body` *"built by a function that does not take a hash argument"*. The same
  shape here: rule that a recovery-wrap outbox entry holds the wrap **plaintext** and MUST NOT hold a
  sealed recovery-wrap record or a `ct_xwing`, and give the republish path one function that takes the
  plaintext and the target's X-Wing public key and does its own encapsulation. Then reusing `ct_xwing`
  is not forbidden, it is **not representable**. That is a data-structure ruling and it is not made here.
- **Re-encapsulating forces the signature to be recomputed, and this is the part easiest to miss.** A
  fresh `ss` gives a fresh `ct_xwing` and a fresh `aead_ct`, so `hybrid_ct` changes, so `ct_body`
  changes, so `body_hash` changes. Part (4) **below**'s signature over the wrap body MUST be recomputed
  and MUST NOT be copied — a republisher that copies the stranded signature ships a signature over bytes
  that are no longer there. And step 6 above lets **any member** repair a fan-out, so a repairer that is
  not the original committer signs under its **own** `identity` key and the restorer must accept that.
  Neither sentence is written anywhere today.
- **It also costs a publisher-side RETENTION obligation, which the 2026-09-14 price omitted entirely by
  calling itself client-side.** The input to a re-encapsulation is the wrap plaintext, and MASTER §8.2's
  payload table says what that is: **`storage_root[k]` ‖ `archive_secret[k]`**, where
  `archive_secret[k] = sender_data_secret[k] ‖ encryption_secret[k]`. Holding it in an outbox past epoch
  *k* is a copy of that epoch's whole non-`EPH` key material living outside the MLS state store and
  outside `record_key[i]`'s overwrite discipline, against `connect/mls/group.go:2488`'s *"THE DELETE IS A
  SECURITY REQUIREMENT AND NOT HOUSEKEEPING"*. **It is not, however, a breach of MASTER §8.1's
  disappearing-message promise** — that promise is `eph_root`'s, and `eph_root` is *"never wrapped to a
  recovery key"* and is not in this payload. How long a stranded wrap may sit in an outbox is a
  forward-secrecy ruling with a measurable cost on the publisher side, and it is the quantity the
  restorer's bound should be derived from.
- **A BOUND MUST BE NORMATIVE, NO DOCUMENT CARRIES ONE TODAY, AND THE PARAGRAPHS ABOVE DESCRIBE AN
  UNBOUNDED WALK.** Stated in that order because the 2026-09-14 text put the bound under *what a retry
  would take* while putting the walk into descriptive prose, so a reader building to this section today
  builds a search with no stopping rule: handed a record it cannot open — corrupt, foreign, or for an
  epoch it is not owed — a restorer cannot distinguish *wrong guess* from *not mine*, and walks back to
  epoch 0 on every such record. **Until item 142 is ruled, the walk above is a derivation and not a
  licence, and a conforming client does not republish at all.**
- **`PastEpochWindow` = 32 is NOT that bound, and the 2026-09-14 text was wrong to reach for it.** It
  was offered as *"the ceiling the corpus already supplies"* on the ground that past 32 epochs
  `DeleteGroupStateBefore` has taken `archive_secret[k]` and no member can **rebuild** that epoch's
  wrap. That argument bounds **rebuilding from live group state**, and a republish is not a rebuild: the
  wrap was produced at epoch *k* while the state existed, and what republishes it is an **outbox**.
  `DeleteGroupStateBefore` is called off `self.context.Epoch` inside the commit-apply path
  (`connect/mls/group.go:2594`, its cutoff guard at `:2587`) and reaches the MLS state store;
  `grep -rni outbox connect/mls/` names no file. A committer whose outbox survives a forty-epoch offline
  stretch republishes at lag 40 and a restorer bounded at 32 refuses to look that far — which loses the
  wrap in exactly the case the retry exists for, silently, because nothing detects a missing recovery
  wrap. The two numbers also share no premise: `PastEpochWindow`'s own comment derives 32 from *"the
  window is a product promise"* / *"about how long a laptop may stay closed, and an active group can
  burn eight epochs in a"* — two strings, because that comment wraps mid-clause
  (`connect/mls/key_schedule.go:25-29`) — while a search bound is a per-unopenable-record work budget
  spent on records an attacker can submit. And the error is one-sided: too small loses a
...
- **And there is a candidate that deletes the question instead of answering it, priced here because the
  ...
  re-seal, no bound, no search — and `expected_wrap_count` becomes a real count over the recovery arm,
  which is the one thing every other route leaves open (item **138**). Its landed-code price was
  measured rather than argued: one arm of one switch (`store/memory.go:939`, shared with `pgx.go:1152`)
  plus `exempt: true` on one map entry (`store/contract.go:2576`), and exactly **one** failing test leaf,
  `OnlyTheExemptKindsPassTheGateWhileTheFanOutIsOpen/AttachmentRecovery`, which fails by **assertion**
  and not through `attachmentKindsDeclared`'s by-name guard — that guard fires for a **new** kind, which
  is a different proposal. Its real price is **availability**: the group cannot write for the recovery
  arm's ~18 extra round trips on every commit, which is exactly what ruling 1 bought back, and it widens
  the interval in which a permanent refusal leaves `epoch_complete = false` with no commit path out. And
  it does **not** make the seal question moot — step 6 above still has any member rebuild a fan-out its
  committer abandoned, and that rebuild re-seals. **Not ruled here either;** it reverses a ruling and
  belongs with 142.
...
`env_key[k]` takes the place of the class key at the head of §5.3's **existing** record ladder, for a
device-wrap record and for nothing else:
`record_key[0] = HKDF-Expand(env_key[k], "sender/v1" ‖ LP(leaf_index), 32)`, with `RecordKeyNext`,
`RecordAeadHead` and `RecordAeadBody` beneath it exactly as §5.3 declares them. **No new ladder is
introduced and no label below the root changes.** It is not circular — `env_key[k]` descends from the
MLS key schedule and from no `storage_root` — and a member the commit *added* holds it the moment
`JoinFromWelcome` returns.

Chosen for the one property nothing else has: `env_key[k]` is the only outer key that **neither the
message server nor a member removed by the commit that opened epoch k** can derive.

**(2) The recovery wrap cannot use the envelope, and is KEM-sealed because nothing else can serve it.**

This is a **consequence** of (1), not a second ruling, and it follows by necessity rather than by
preference. The recovery wrap's only intended reader is a seed-only restorer (§5.4), and MASTER states
in as many words that such a restorer *"has none by definition"* — no MLS state, therefore no
`mls_secret[k]` and therefore no `env_key[k]`; and no `storage_root` of any epoch, which is the whole
of why correction **E1** exists. Any outer key derived from MLS state or from any `storage_root` makes
...
**The head is not optional, and the reason is measured rather than stylistic.** A zero-length `ct_head`
is refused at `msgrepo/api/submit.go:301`, refused again at `store/memory.go:979`, its column is
`NOT NULL` (`store/migrations.go:160`), and `store/contract.go:145`'s `ARecordWithNoHeadAtAll` asserts
that refusal against **both** stores. A recovery wrap with no head is rejected by the shipped server,
every time. `wrap_key` comes from decapsulation, so keying the head off it is not circular, and it
preserves the property the KEM seal exists for: the only key that opens a recovery wrap is the target's
own.
...
by nothing in this section. *(**M1-6 was ruled 2026-09-07 and REVERSED 2026-09-13**, and this parenthesis is corrected rather
than re-pointed because it stated the reversed rule as a fact about this record. It read: *"the
snapshot is a `PERMANENT` record, so its `ct_head` is sealed under the **DURABLE** class ratchet and
its body — it has none — would have been under `K_perm`."* **Under the 2026-09-13 ruling the snapshot's
`ct_head` is sealed under `K_perm`, its own class key**, and there is no second ladder for it to
straddle. The ladder **position** is still ledger **143** and **169**'s shape **A1** — position
`stream_index` out of the sender's one class-blind counter — and that is untouched, so this record
owes nothing further. §5.3 and §5.6 carry the rule.)* *(**The `56` is MASTER's 2026-09-19 amendment**, and it changes nothing
...
requires every field a client validates to be inside the MLS-authenticated payload or covered by
`write_auth`; and §2.4 empties that second arm on the read path — `write_auth` is zero on read and a
client MUST NOT verify it on a fetched record. Without the signature, a wrap's epoch, `sender_handle`,
`stream_index`, retention class, size bucket and `wrap_target_handle` — every field a client validates
and acts on — are authenticated by nothing, and MASTER §9.2's stated mitigation for server injection,
*"any record it injects fails MLS verification at every client (I5)"*, has no referent for a wrap.
64 octets fit inside the rung with about 2.8 KB to spare; the sizing paragraph carries it.
...
is header **forgery**. (*Corrected 2026-09-15: this sentence ended* **"and not a plaintext XOR"** —
*the word* `forgery` *is bolded in the source, so that is the grep-able half — and the claim holds only
where the two head plaintexts are byte-identical. A repeated
`(key, nonce)` repeats the keystream, so two heads that differ at all also yield their own XOR — and
the bullet above records that nothing states whether a wrap's head carries a value like `sent_at` that
moves between rebuilds. Forgery is the worse half and it is why the sentence names it; it is not the
only half.*) `i` and `stream_index` are not the same thing by default: gaps are normatively legal
(§5.6, *"the server enforces monotonicity, not contiguity"*), and one rate-limited submit mid-fan-out
is enough to separate them. **Rulings 2 and 3 sharpen this rather than soften it.** Ruling 2 gives the
device wrap its own ladder head, `record_key[0] = HKDF-Expand(env_key[k], "sender/v1" ‖ LP(leaf_index),
32)`, and ruling 3 puts **two** records per leaf on that ladder, so a fan-out now advances it twice per
leaf where it advanced it once. The repair the review proposes is to pin `i = stream_index` in every
ladder, so the invariant follows from §5.12 step 6's existing *"MUST NOT be reused"* rule rather than
...
LP(leaf_index), 32)`, and the **class key is what separates one sender's ladders from each other** —
which is why §5.5 sizes the skipped-key window *"per (`sender_handle`, retention class)"* and why §5.3
publishes `NewSenderRatchet(classKey []byte, leaf uint32)` and `RecordKeyZero(classKey []byte, leaf
uint32)`, signatures with a class-key parameter and nowhere to put anything else. Ruling 2 replaces
`class_key` with `env_key[k]`, which has **no class in it**, and ruling 3 then puts a `PERMANENT`
record (`pq_secret[k]`) and an `EPH(5)` record (`eph_root[k]`) on that one root. An implementer that
adapts the published constructor by passing `env_key[k]` as `classKey`, and instantiates one ratchet
per (sender, class) as §5.5 directs, gets **two ratchets with byte-identical roots, both starting at
`i = 0`** — so the two device wraps for one leaf are sealed under the same `(key_head, nonce_head)`
and the same `(key_body, nonce_body)` with different plaintexts and different AADs, and the message
server recovers `pq_secret[k] ⊕ eph_root[k]` for every leaf of every epoch. That is simultaneously a
...
> `pq_secret[k] ⊕ eph_root[k]`. *(**Sealing the `EPH(5)` half was refused under ledger item 152 until
> 2026-09-13, when 152 was ruled and the refusal lifted in full (§5.3, revision A-25).** The clause
> here read *"Sealing the `EPH(5)` half is still refused, under ledger item **152**, which is a
> different question."* It was a different question and it is now an answered one: an `EPH` record's
> class key is `EphKey(ephRoot, b, window)`. **Nothing in this paragraph's own argument moves** —
> A1's two positions on the shared `env_key[k]` root are what separate the two device-wrap records,
> and that root carries no class, so the 2026-09-13 reversal does not reach it.)*
...
epoch k.** Past-epoch state is retained for thirty-two epochs, but no published API reaches a past
epoch's exporter, so the window for computing `env_key[k]` is the live epoch and nothing longer.

Therefore, normatively:

- **Who caches.** Every client, on every transition into an epoch — its own merged commit, or a peer's
  commit processed during catch-up. On entering epoch *k*, and **before merging any further commit**, a
  client MUST call `Export("URmessage/v1/envelope", "", 32)` and retain `env_key[k]`.
- **For how long.** Until it has opened its own device-wrap records for epoch *k* and derived
  `storage_root[k]`, at which point `env_key[k]` is zeroized. `storage_root[k]` is the value it needed;
  `env_key[k]` is only the way in.
- **Where.** It MUST survive a restart. A client that merges epoch *k* and restarts before fetching its
  wrap has otherwise lost `storage_root[k]` permanently, so `env_key[k]` is durable client state and
  not a value on the stack. **Which store holds it is not ruled here.**
- **A client walking a backlog** of *m* commits performs *m* exports and holds up to *m* cached keys.
  There is no batch form and no recomputation.
- **What a client does when it has missed the window.** The honest answer is that it cannot recover:
  **`storage_root[k]` is unrecoverable for that device at that epoch**, and with it every class key of
  epoch *k*, every record sealed under them, and that epoch's snapshot (`K_snapshot[k]`). It is **not**
  locked out of the group — `read_key[k]` and `write_key[k]` arrive in the clear in the commit's
  `EpochAttachment`, so it still fetches and still submits — and it MUST surface the epoch as a visible
  failure rather than failing silently or retrying forever. **Which member of `GapReason`'s closed set
  carries that failure, or whether the closed set gains a seventh member, is not ruled here** and is
  filed as an open item.
- **One undetermined fact could change that answer, and it is not assumed here.** If §5.4's provisioning
  bundle gives a linked device the master key, that device also holds `recovery_root` and the recovery
  X-Wing private key, and could open its **own recovery wrap** at epoch *k* to obtain `storage_root[k]`
  outright, skipping the device wrap entirely. Nothing in the corpus states whether it does. Filed;
  until it is determined, the paragraph above is the answer.

**The device wrap is two records (RULED 2026-09-13).**

| Record | Class | Carries | Rung |
|---|---|---|---|
| the `pq_secret` device wrap | `PERMANENT` | `pq_secret[k]`, 32 octets | `size_bucket 2` |
| the `eph_root` device wrap | `EPH(5)` | `eph_root[k]`, 32 octets | `size_bucket 2` |

Both carry a `WrapTag`; both are sealed as (1) says; both are signed as (4) says; and both are indexed
by the **same** `wrap_target_handle`, because the derivation at the head of this section is unchanged
and no ruling of 2026-09-13 changes a derivation.

**Why.** MASTER §8.1's disappearing-message promise — *"after the timer, retained server ciphertext, a
seized device, a newly provisioned device, and a seedphrase holder all fail to decrypt"* — holds today
only because clients agree to delete, and §8.1 already calls it *"the most easily broken property
here."* *(**The quoted sentence was NARROWED in MASTER on 2026-09-13, second pass of that date**: it
now lists three adversaries and not four, because nothing schedules `eph_root[n]`'s destruction on a
member device — **ledger open item 186**. The quotation is kept as the sentence this ruling was argued
against, and the argument below is unaffected: it is about a device X-Wing key that never rotates
opening every retained wrap, which is a different path from a seized device's own epoch state.)* A single device wrap is `PERMANENT` and prunable **never** (Spec B §3.5), and the key it is
encapsulated to never rotates: `ProposeUpdate`'s own header (`connect/mls/group.go:1613`) states that
*"The device wrap key is read off the leaf being REPLACED and re-encoded, so an update publishes the
same `urmessage_leaf_keys` the group already wraps to."* One device X-Wing key, obtained once at any
point in a device's life, therefore opened every retained device wrap of every epoch, hence every
`K_eph[k][b][t]` that ever existed. Splitting the record puts `eph_root[k]` on a rung the server
actually prunes. It is the only construction offered anywhere that makes §8.1's promise
**cryptographic rather than behavioural**, and it closes ledger open item 136.

**The accepted costs, all three of them, written where they will be read.**

- **The device-wrap count doubles** — 2,000 records rather than 1,000 at the 500-member × 2-device
  target — and the bundle grows with it. The sizing paragraph below carries the arithmetic.
- **Two records now land at one `wrap_target_handle`, by design.** A `WrapFetch` by target returns
  both, and a client separates them by the retention class the record header already carries. This
  makes *two wraps at one handle* the **normal** case rather than the attack signature ledger item 132
  treats it as, and it forecloses the uniqueness constraint on `(group_id, epoch, wrap_target_handle)`
  that item 132's proposed repair reaches for, unless that constraint also takes the retention class.
  **Recorded as an interaction, not resolved.** The alternative — a distinct handle label for the
  `eph_root` record — would change a derivation, and no ruling here changes one.
- **The `eph_root` wrap's expiry is measured from its publication, not from its epoch's end.** It is
  published at the start of epoch *k* on the EPH(5) rung (2,419,200 s, twenty-eight days), while an
  EPH(5) record written *later* in the same epoch expires later. A device that has not opened the
  `eph_root` wrap before the server erases its body loses the tail of that epoch's ephemeral traffic,
  and the gap widens with the epoch's lifetime. Whether that wrap's rung should instead track the
  epoch's own lifetime is **not ruled here** and is filed.
- **AND THE `eph_root` WRAP'S OWN `eph_window` VALUE IS NOT RULED. A builder MUST NOT PUBLISH THAT
  RECORD UNTIL IT IS — ledger open item 185, filed 2026-09-13, second pass of that date.** Three
  landed sentences cannot all be satisfied by it. MASTER §8's presence rule makes `eph_window`
  non-zero on `EPH(1..5)`, and this record is `EPH(5)`, wire byte `0x15`. Requirement **S19** below,
  Spec B §5.1 check 3 and Spec B §7.1 all refuse an `EPH(1..5)` record whose window differs from its
  arrival window by more than one, **with no carve-out for a wrap anywhere**. And the value's own
  formula is `floor(sent_at_ms / …)`, while part **(5)** above states that a wrap head's plaintext is
  **not stated** — a wrap carries no MLS frame, so it has no `sent_at` to divide. A builder writing
  `0` is refused and the epoch fan-out stops, which is `expected_wrap_count`'s *"exactly two"* failing
  with no device ever obtaining `eph_root[k]`; a builder computing from its own publication clock is
  equally conforming on the text as it stands. What is owed is one sentence: what `eph_window` an
  `EPH(1..5)` record that is **not keyed under `K_eph`** carries, and whether **S19** applies to it.
  The `pq_secret` device wrap is unaffected — it is `PERMANENT` and carries `0`.
...
It counts **both** device-wrap record kinds and the snapshot, and it counts **no recovery wrap**. At
the design target that is 2 × 1,000 + 1 = **2,001**, and the marker's `wrap_count` must equal it.

**And for the recovery arm it is decorative, by construction.** The count names a set that closes when
the marker lands, and under the resequence above the recovery wraps are published after that moment —
so the count cannot name them, and the omission detector every M1-1 option leaned on cannot see a
...
- **Step 5's `no_wrap` gap cannot.** It is keyed on *"after the marker has landed"*, and after the
  marker has landed an absent recovery wrap is indistinguishable from one that has not been published
  yet. That detector is sound for the device arm and the snapshot, which land before the marker, and
  unsound for the recovery arm.
...
for that member.** It is filed as an open item rather than repaired here, because repairing it means a
coverage check the server can actually make, which is item 132's and is not m1's to rule.

**Sizing at the 500-member × 2-device design target, after the split.** Wraps pad to the ladder like
everything else, and the arithmetic is MASTER §7's framing with `LP` = 4 octets,
...
wraps per submission, so the wrap traffic takes **~90 round trips**, and it is now split either side of
the marker: **~72** for the device arm and the snapshot, then the marker, then **~18** for the recovery
arm.
...
populate one, and separating them would leave the adapter unable to stage a commit unforgeably.

```go
// connect/messagegroup/engine.go
//
// the entire MLS surface the storage layer is allowed to see. adding a method
...
without `connect/messagegroup` being able to read or forge it.

**The scope of that guarantee, stated because the loose reading of it has been wrong once.**
`stagedRef` being unexported confines **populating** it, not implementing the interface. A keyed
composite literal naming only the exported fields is legal across package boundaries, so a
replacement engine may be declared anywhere and satisfy `GroupHandle`; what it cannot do is put
anything in `stagedRef`, so it must carry its staged commit in `Raw` — where
`connect/messagegroup` can read and forge it — or in state of its own. The unforgeability above
therefore holds **for engines declared in `connect/messagegroup`**, and a foreign engine trades that
guarantee for its independence. Gate 5's swap is unaffected: it is a change to the factory below.
...
type MlsEngineFactory interface {
    NewEngine(store MlsStateStore, signer *MessageIdentity) (messagegroup.GroupEngine, error)
}
...
a claim that it is the only one). **A destroyed key is `"expired"` and not `"malformed"`**, which is
the distinction that matters here: a window *behind* the opener's is never a refusal in any amount. Attachment outcomes are
...
| `eph_root[n]` | SQLite `eph_root`, inside the epoch state row | **yes** | **`DeleteGroupStateBefore`, epoch window 32 — and NOTHING ELSE today.** *(Amended 2026-09-13, second pass. This read "window closes, or epoch falls out of the window"; after that date's window ruling `eph_root[n]` is one per-epoch value serving buckets 0..5 with `t` as an HKDF `info` term, so there is no single window of its own to close. A destruction schedule in the ruling's own unit is **ledger open item 186**, filed and not ruled; until it is ruled this row states what the store actually does.)* |
...
`replace ../` in `server/go.mod` — and, since §2.2's split, **that package and nothing beside
it**. `github.com/urnetwork/connect/messagegroup` holds the key schedule, both ratchets, the
session, the sealer, X-Wing, the wraps and the MLS adapter, and Spec B §2.2's allow list does not
carry it, so a server that reached for one of them fails the message server's own dependency gate by
name rather than by review. This table is the single published version of that surface and is
...
**Two of those three are now facts about the binary rather than rules about the reader.** After
§2.2's split the key schedule and every MLS type are in `connect/messagegroup`, which the message
server does not link and which its dependency gate does not allow; reaching one is a build the gate
refuses, not a review it passes. The third — *no decryption function* — is still a rule about
this list, because `connect/message` exports more than this list does: `AADHead`, `AADBody` and
`BodyBinding` build MASTER §8's two record AEAD preimages and are deliberately on no line here
(A-9). This block has never been the package's export set, and the split does not make it one. The
allowlist test is what holds the difference, and it is still unwritten.
...
| S5 | Index epoch wrap records by target: device-wrap records and the epoch snapshot by `wrap_target_handle` (16 B, from `WrapTag`), recovery wraps by `recovery_handle` (16 B, from `RecoveryTag`), both delivered inside the authenticated `server_attachment`. Serve a wrap by target in O(1). **Since 2026-09-13 a leaf has TWO device-wrap records at one `wrap_target_handle`** — a `PERMANENT` one and an `EPH(5)` one (§5.11) — so a serve-by-target returns both and the index MUST NOT assume one row per target. | §5.11, MASTER §8.2 |
| S6 | A commit's epoch bundle reaches **~11.5 MB** at 500 members after the 2026-09-13 device-wrap split (~6.9 MB before it), every wrap padded to `size_bucket 2`. Per-record size caps apply to individual wrap records, never to the bundle. Honour the epoch-publication sequence of §5.11 — including that the recovery wraps land **after** the `EpochComplete` marker, not before it. | §5.11 |
| S7 | Serve a `FetchAttestation` on every history fetch, with the field list and signing preimage of MASTER §9.4 — including `class_mask`, `heads_only` **and `read_epoch`** inside the signature. The third joined on 2026-09-22 with the epoch ceiling (Spec B §5.1.1): it is what keeps an honest short-ceiling answer distinguishable from a withholding one, now that `high_water_record_id` is ceiling-relative | MASTER §9.4 |
...
| S10 | Prune by retention class **and** `expire_at`, where `expire_at` may only shorten retention, never extend it. **Head retention is per class and not universal:** retain `ct_head` and `body_hash` when `ct_body` is erased for **`PERMANENT`, `DURABLE` and `MEDIA`**; for **`EPH(1..5)`** set `ct_head = NULL` and **zero** `body_hash` in the same statement that erases the body, leaving S16's ~60-byte placeholder row. `EPH(0)` is never persisted and has neither field on disk. **AMENDED 2026-09-11.** This row read *"retain `ct_head` and `body_hash` when `ct_body` is erased"* with no class scope, which is false for `EPH(1..5)` on **both** clauses — Spec B §7.2 — and which disagreed with **all three of the sources named beside it**: MASTER §8's record listing excludes `EPH(1..5)` from both fields in its own voice since the amendments of 2026-09-11, MASTER §12.2's *"What an expired disappearing message leaves behind"* enumerates `record_id`, `epoch`, `retention_class` and `size_bucket` and names **neither** field, and MASTER §9.1 requires pruning by class and says nothing about retaining either. A server implementer works from this table, so this row is the most directly actionable voice the unqualified premise had anywhere in the corpus. Revision **A-24** | MASTER §8, §9.1, §12.2; Spec B §7.2; A-24 |
...
| S19 | **Refuse an implausible `eph_window`.** For an `EPH(1..5)` record, compute the window its own arrival stamp falls in — `floor(create_time_ms / (eph_bucket_seconds[b] × 1000))`, with `b` read from the plaintext `retention_class` — and refuse the record when the submitted `eph_window` differs from it by more than **one**, in either direction, with `REASON_REJECTED`. The submitted value is inside the `write_auth` preimage, so this is a check on a value the MAC covers. **±1 and not tighter** because the sender computes from `sent_at` and the server sees arrival, which is the skew §7.1's one-hour `grace` already absorbs, in this field's unit; **±1 and not looser** because two windows doubles the shortest bucket's guarantee. A window far in the future is the case that matters: under the 2026-09-13 ruling it extends the record's own KEY past its timer, so no later sweep corrects it. **This requirement has NO carve-out for a wrap record, and the `eph_root` device wrap is `EPH(5)` — whose `eph_window` value no document states. Whether S19 applies to a record whose `eph_window` selects no key is ledger open item 185, filed 2026-09-13 (second pass) and not ruled; until it is, S19 is written as it stands and §5.11 refuses the publication rather than this row refusing the check.** | MASTER §8.1, §9.2; ledger 152 / M1-27, ruled 2026-09-13; ledger 185 |

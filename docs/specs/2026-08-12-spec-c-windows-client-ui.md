...
| 7 | **A BACKFILL, AND THE ONLY ROW HERE THAT IS NOT THE COMMIT IT DESCRIBES: the three commits this log was owed — `d2e7a51`, `368ef8d`, `21f22c7` — and the mechanical gate that takes the next one.** Nothing in this document is amended by this row. It ended at Rev 6 while the document moved three times after it, and every one of those three moves was a normative line. **`d2e7a51`** — §8.1 gains a note, placed **outside every quoted value** so lint 1 is unaffected, that `msg_disappearing_explainer` **is not changed and must not be** and that what discharges it is not fully written down: the string promises destruction *"on every device and on the server"*, the server half is discharged because the message server never holds `eph_root[n]` at all, and the device half has **no schedule anywhere in the corpus**. **`368ef8d`** — §5.1's `Gap` rendering row and §14.2's `gap` entry-kind line gain the reason **`unsupported`**, the value that makes every later content kind additive rather than a format break: a record that opened and verified but carries a kind code this build does not know is a newer feature, and telling the user it is `malformed` would accuse a correct sender. **`21f22c7`** — §9.6's rule that attestations are compared only within an identical filter becomes the tuple **`(class_mask, heads_only, read_epoch)`** (ruling 32), because a ceiling-clamped fetch and a withholding one were otherwise byte-indistinguishable. **Why a backfill and not a reminder:** `editlog_test.go` now holds every commit that changes this document to adding a row **inside §0.6**, measured off the file as of that commit; the three hashes above are dispositioned `backfilledInTheLog` there and that disposition is asserted **against this row** — the test fails unless each hash appears in this region. `3bc0603`, which created this document, is dispositioned `createdTheDocument`, asserted by this table beginning at **Rev 2** — and it is also the row that shows why the gate scopes an added line to this section rather than to the diff: that commit added **32** lines matching this table's row shape and not one of them is an edit-log row. |
...
**`msg_disappearing_explainer` IS NOT CHANGED AND MUST NOT BE, AND WHAT DISCHARGES IT IS NOT FULLY WRITTEN DOWN.** *(Noted 2026-09-13, second pass of that date; **no string in this section is edited** and lint 1 is unaffected, because this note sits outside every quoted value.)* The string promises key destruction *"on every device and on the server"*. The server half is discharged — the message server never holds `eph_root[n]` at all. **The "on every device" half has no schedule anywhere in the corpus**: after the 2026-09-13 window ruling, `t` slices the *derived* key `K_eph[n][b][t]` and `eph_root[n]` is one per-epoch value with no window of its own, so no document says when a member device destroys either. **Ledger open item 186**, filed and not ruled; MASTER §12.4 and §8.1 carry the same note at the sentences that make the same claim. This is recorded here because this document is where the promise reaches a user, §14's *"an ephemeral message's timer expiring — the key is destroyed on every device and on the server"* repeats it in prose, and neither would have been reached by any query run over `docs/specs/*.md docs/plans/*.md SPEC-LEDGER.md` for `ct_head` or `durable`.

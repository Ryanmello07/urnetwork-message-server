...
// The database's wall clock agrees with this process's, to within `slack`, **through the pool**.
//
// **This is a SKEW check and it is named for what it measures.** It used to be called CheckClock
// and to be documented as "the database clock reads UTC", and it could never have found a cluster
// that was not UTC: [NewPgxPool] pins `timezone = UTC` on every connection it hands out,
// `now()::timestamp` renders in the SESSION's zone, and so this comparison is against a zone the
// startup packet has just set to the one it is checking for. It was met on every run of a suite
// whose own cluster was `America/Phoenix`, seven hours out — the check was not lenient, it was
// blind by construction. [CheckClusterTimezone] is the one that is not.
//
// What this does find is a real and different fault: the two machines disagree about the
// instant. §7.1 computes `prune_after` from `time.Now().UTC()` in Go and §7.4 sweeps
// `WHERE prune_after <= now()` in the database, so a database clock ahead of this process prunes
// user data early even when both are nominally UTC.
func CheckClockSkew(ctx context.Context, pool *pgxpool.Pool, slack time.Duration) error {
	var stamp time.Time
...
// The CLUSTER's own session timezone is UTC — §3.1's actual requirement, and §13 item 21's
// "`/readyz` fails on a cluster whose timezone is not UTC."
//
// **It opens its own connection, and that is the whole mechanism.** The value cannot be read
// through [NewPgxPool]'s pool, and not for want of trying: inside a session whose startup packet
// carried `timezone`, `RESET TimeZone` returns to that value and not to the cluster's,
// `pg_settings.reset_val` is that value too and `source` is `client`, and `pg_file_settings` —
// which does carry the row, under the name as spelled in postgresql.conf — needs a privilege the
// message-server role has no reason to hold and sees only config FILES, not `ALTER DATABASE`,
// `ALTER ROLE` or a `-c` on the command line. All four were measured on a cluster set to
// `America/Phoenix`. A connection that never sends the parameter is the only thing that sees what
// the cluster would have done.
//
// **The predicate is an offset and not a name, and a year and not an instant.** A name test would
// have to carry a list of the spellings PostgreSQL treats as UTC, and would still pass
// `Europe/London`, whose offset is zero every January and one hour every July — a fleet that
// prunes correctly all winter and an hour early all summer. A single reading passes it for half of
// each year. So what is asked is: does this session render every month of the current year exactly
// as UTC renders it. The twelve instants are generated by the database from `now()` rather than
// written here, so the check cannot age into a pair of stale literals.
//
// Measured, on a real cluster, one session zone at a time: `UTC` and `Etc/UTC` pass;
// `America/Phoenix` fails every month; `Europe/London` and `Atlantic/Azores` each fail the half of
// the year they are not at zero, in opposite directions; and `Africa/Casablanca`, which is at zero
// only for Ramadan, fails too.
//
// There is deliberately no separate reading of the current instant beside the sweep. One was
// written and then removed: `now()`'s own month is one of the twelve, and every sample is rendered
// in the session's CURRENT zone, so deleting it turned no test red — it defended nothing while
// reading like a safeguard.
//
// It is a readiness check and not a startup one, deliberately, and this is the placement's whole
// justification: the case that actually produces a non-UTC cluster is a failover to a streaming
// replica configured in another zone, which happens to a process that is already running and
// which a startup check could not see. That is also why the answer is never cached.
func CheckClusterTimezone(ctx context.Context, dsn string) error {
	// pgxpool.ParseConfig and NOT pgx.ParseConfig, which is a distinction with teeth: the `pool_*`
	// keys are pgxpool's, and [NewPgxPool]'s own comment sends an operator to put their sizing in
	// the DSN — the ops document's example `pg.yml` carries `pool_max_conns=16`. pgx's parser does
	// not know those keys, forwards them to the server as runtime parameters, and the connection
	// dies with `unrecognized configuration parameter "pool_max_conns"`. That is not hypothetical:
	// it is what this function did on its first draft, against the DSN shape the documentation
	// tells people to write.
	poolConfig, err := pgxpool.ParseConfig(dsn)
	if err != nil {
		// pgx puts the DSN in this error and the DSN carries a password
		return errDsnUnreadable
	}
	config := poolConfig.ConnConfig
	// the one line this function exists to undo. Without it this connection is the pool's
	// connection, and answers the question it was told the answer to.
	delete(config.RuntimeParams, "timezone")
	...
	var everyMonthIsUtc bool
	if err := conn.QueryRow(ctx, `
        SELECT bool_and(month::timestamp = (month AT TIME ZONE 'UTC'))
          FROM generate_series(date_trunc('year', now()),
                               date_trunc('year', now()) + interval '11 months',
                               interval '1 month') AS month`).
		Scan(&everyMonthIsUtc); err != nil {
		return err
	}
	if !everyMonthIsUtc {
		return errClusterNotUtc
	}
	return nil
}

var (
	errClockSkew     = errors.New("store: the database's wall clock and this process's disagree by more than the slack, and §7.1 and §7.4 read one each")
	errClusterNotUtc = errors.New("store: the cluster's own timezone is not UTC, and §7.4 would prune against it")
	errDsnUnreadable = errors.New("store: the DSN did not parse")
)
